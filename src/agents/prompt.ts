...
export function buildAgentCatalogPrompt(agents: AgentDefinition[]): string {
  ...
  return [
    ...
    "- If the user asks to inspect, summarize, coordinate, or consolidate child subagent work for the current parent session, use agent.inspect_children.",
    "- If the user asks to inspect background subagent tasks, use agent.list_background_tasks when appropriate.",
    "- If the user asks to run, continue, process, or execute a queued background subagent task, use agent.run_background_task when a backgroundTaskId is available.",
    ...
    "- agent.inspect_children can inspect child subagent work for the current session without requiring parentSessionId.",
